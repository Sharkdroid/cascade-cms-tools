...
Both sync the *installed* cascade-cms-rest package into the bundle (this repo
holds no copy of the library's source), record a manifest, validate every
...
to be small, so growth is a build failure, not a judgement call.
    ...
    python skill/build_skill.py                  # both, version from installed cascade-cms-rest
